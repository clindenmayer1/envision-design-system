...
`;
const MODES = ['customize', 'packages'];
...
export class EnvisionRightRail extends EnvisionElement {
    ...
    #mql = null;
    #returnFocusTo = null;
    get mode() { return this.getEnum('mode', MODES, 'customize'); }
    set mode(v) { this.setStr('mode', v); }
    get loading() { return this.getBool('loading'); }
    set loading(v) { this.reflectBool('loading', v); }
    get open() { return this.getBool('open'); }
    set open(v) { this.reflectBool('open', v); }
    /** Force sheet presentation; otherwise auto below 1024px. */
    get sheet() { return this.getBool('sheet'); }
    set sheet(v) { this.reflectBool('sheet', v); }
    connectedCallback() {
        if (!this.hasAttribute('mode'))
            this.setAttribute('mode', 'customize');
    ...
    #focusables() {
        const tabs = Array.from(this.shadowRoot.querySelectorAll('envision-tab'));
        const slotted = Array.from(this.querySelectorAll(FOCUSABLE)).filter((el) => !el.hasAttribute('disabled'));
    ...
    updated() {
        ...
        if (this.hasAttribute('data-sheet') && this.open) {
            ...
            if (first && this.shadowRoot.activeElement !== first)
                queueMicrotask(() => first.focus());
        }
    }
}
//# sourceMappingURL=RightRail.js.map