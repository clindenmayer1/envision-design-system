 ...
 */
export function tryAttachInternals(el: HTMLElement): ElementInternals | null {
  const attach = (el as unknown as { attachInternals?: () => ElementInternals }).attachInternals;
  if (typeof attach !== 'function') return null;
  try {
...
export function setFormValue(internals: ElementInternals | null, value: string | null): void {
...
export function emitChange(host: HTMLElement, detail: unknown): void {
  host.dispatchEvent(new CustomEvent('change', { bubbles: true, composed: true, detail }));
}

export function emitInput(host: HTMLElement, detail: unknown): void {
