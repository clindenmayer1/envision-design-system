...
export class EnvisionInput extends EnvisionElement {
  ...
  get type(): FieldType { return this.getEnum('type', TYPES, 'text'); }
  set type(v: FieldType) { this.setStr('type', v); }
  get value(): string { return this.#input ? this.#input.value : this.getStr('value') ?? ''; }
  set value(v: string) { this.setStr('value', v); }
  get label(): string { return this.getStr('label') ?? ''; }
  set label(v: string) { this.setStr('label', v); }
  get helperText(): string { return this.getStr('helper-text') ?? ''; }
  set helperText(v: string) { this.setStr('helper-text', v); }
  get errorMessage(): string { return this.getStr('error-message') ?? ''; }
  set errorMessage(v: string) { this.setStr('error-message', v); }
  ...
  protected updated(): void {
    ...
    if (t === 'price' || t === 'number') this.#input.inputMode = 'decimal';
    ...
    if (ph) this.#input.placeholder = ph; else this.#input.removeAttribute('placeholder');
