...
export class EnvisionOptionCard extends EnvisionElement {
  ...
  get options(): MaterialOption[] { return this.#options; }
  set options(v: MaterialOption[]) { this.#options = v ?? []; this.requestUpdate(); }
  get value(): string { return this.#value || (this.getStr('value') ?? ''); }
  set value(v: string) { this.#value = v; this.requestUpdate(); }
  get title(): string { return this.getStr('title') ?? ''; }
  set title(v: string) { this.setStr('title', v); }
  get note(): string { return this.getStr('note') ?? ''; }
  set note(v: string) { this.setStr('note', v); }
  get active(): boolean { return this.getBool('active'); }
  set active(v: boolean) { this.reflectBool('active', v); }
  ...
  #onClick = (e: Event): void => {
    if (this.getBool('loading')) { e.stopImmediatePropagation(); return; }
  ...
  protected updated(): void {
    ...
    if (!image && color) this.#thumb.style.background = color;
